...
def insert_tweet(connection,tweet):
    ...
    with connection.begin() as trans:
        ...
        try:
            ...
            try:
                geo_coords = '('  # Start with single paren - loop will add the second
                for i,poly in enumerate(tweet['place']['bounding_box']['coordinates']):
                    ...
                    geo_coords+=')'
                geo_coords+=')'  # End with single paren
        ...
        # insert the tweet
        # Build the geo value based on geo_str type
        if geo_str is None:
            geo_value = "NULL"
        elif geo_str == 'POINT':
            geo_value = f"ST_GeomFromText('POINT({geo_coords})')"
        elif geo_str == 'MULTIPOLYGON':
            # MULTIPOLYGON needs triple parentheses: MULTIPOLYGON(((x y, x y, ...)))
            geo_value = f"ST_GeomFromText('MULTIPOLYGON({geo_coords})')"
